...
def plot_shipment_heatmap(results: Dict[str, Any]) -> None:
    ...
    # Add route labels for first few routes
    route_labels = []
    arc_count = 0
    for i in PORT_INDICES[:5]:  # Limit to first 5 ports for readability
        for j in PORT_INDICES[:5]:
            if i != j and arc_count < 20:
                route_labels.append(f"{PORT_NAMES[i][:4]}→{PORT_NAMES[j][:4]}")
                arc_count += 1
    
    if route_labels:
        plt.yticks(range(len(route_labels)), route_labels, fontsize=8)
